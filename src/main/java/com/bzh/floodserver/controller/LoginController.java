...
import com.bzh.floodserver.core.singleLogin.LoginHandler;
import com.bzh.floodserver.model.request.RequestRegister;
import com.bzh.floodserver.model.User;
import com.bzh.floodserver.service.UserService;
import com.bzh.floodserver.utils.Data;
import com.bzh.floodserver.model.request.RequestLoginUser;
import com.bzh.floodserver.utils.ResultMap;
import com.bzh.floodserver.model.user.LoginDetail;
import com.bzh.floodserver.model.user.TokenDetail;
...
public class LoginController {
	...
	@Autowired
	public LoginController(UserService userService, LoginHandler loginHandler) {
	...
	private ResultMap checkAccount(RequestLoginUser requestLoginUser, LoginDetail loginDetail) {
		if (loginDetail == null) {
			return new ResultMap().fail("434").message("账号不存在！").data(new Data().addObj(tokenHeader, "").
                    addObj("username", "").addObj("state",false));
		} else {
			if (!loginDetail.enable()) {
				return new ResultMap().fail("452").message("账号在黑名单中").data(new Data().addObj(tokenHeader, "").
                        addObj("username", "").addObj("state",false));
			}
			if (!loginDetail.getPassword().equals(requestLoginUser.getPassword())) {
				return new ResultMap().fail("438").message("密码错误！").data(new Data().addObj(tokenHeader, "").
                        addObj("username", "").addObj("state",false));
			}
